#!/usr/bin/env python3
"""
Fixed HTML Context Test - Proper HTML Structure

This test fixes the HTML context building to create proper, valid HTML
that matches the actual element types and structure.
"""
...
class FixedHTMLContextTest:
    """Fixed HTML context test with proper structure."""
    
    def __init__(self):
        """Initialize the test with real MarkupLM model."""
        print("🚀 Initializing Fixed HTML Context Test")
        print("=" * 80)
    ...
    def build_html_context(self, node: Dict[str, Any], all_elements: List[Dict[str, Any]]) -> str:
        """Build proper HTML context for a node with correct structure."""
        # Create a simple, clean HTML structure that matches the element type
        tag = node.get('tag', 'div')
        text = node.get('text', '')
        attrs = node.get('attributes', {})
        attr_str = self.build_attribute_string(attrs)
        
        # Build a clean HTML snippet with proper structure
        html_parts = []
        
        # Add a simple container div for context
        html_parts.append('<div class="element-container">')
    ...
    def run_test(self):
        """Run the fixed HTML context test."""
        print("\n" + "="*80)
        print("🧪 RUNNING FIXED HTML CONTEXT TEST")
        print("="*80)
        
        # Real DOM data from Verizon page (no mocking!)
        real_dom_elements = [
        ...
        print(f"📋 Running {len(test_steps)} test steps with FIXED HTML context")
