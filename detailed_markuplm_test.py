#!/usr/bin/env python3
"""
Detailed MarkupLM Test - Show Full Input/Output

This test shows the complete MarkupLM input and output for each step.
"""
...
class DetailedMarkupLMTest:
    """Detailed MarkupLM test showing full input/output."""
    
    def __init__(self):
        """Initialize the test with real MarkupLM model."""
        print("🚀 Initializing Detailed MarkupLM Test")
        print("=" * 80)
    ...
    def score_snippets_with_markuplm(self, html_snippets: List[str], query: str) -> List[Tuple[float, str, int]]:
        """Score HTML snippets using real MarkupLM model with detailed output."""
        ...
        for idx, snippet in enumerate(html_snippets):
            ...
            print(f"{'='*60}")
            print(f"HTML Length: {len(snippet)} chars")
            ...
            try:
                ...
                print(f"\n🔧 MARKUPLM INPUT PREPARATION:")
                print(f"   Query: '{query}'")
                print(f"   HTML: {snippet[:100]}{'...' if len(snippet) > 100 else ''}")
                ...
                print(f"   Token type IDs: {inputs['token_type_ids'].tolist()}")
                
                # Get model predictions
                print(f"\n🧠 MARKUPLM MODEL PROCESSING:")
                with torch.no_grad():
                    ...
                    print(f"   Start probabilities: {start_probs.tolist()}")
                    print(f"   End probabilities: {end_probs.tolist()}")
                    ...
                    # Show which tokens have highest probabilities
                    start_max_idx = start_probs.argmax().item()
                    end_max_idx = end_probs.argmax().item()
                    print(f"   Start max token index: {start_max_idx}")
                    print(f"   End max token index: {end_max_idx}")
        ...
        for i, (score, snippet, orig_idx) in enumerate(results):
            print(f"   {i+1}. Snippet {orig_idx+1}: Score = {score:.6f}")
    ...
    def run_detailed_test(self):
        """Run the detailed test with MarkupLM integration."""
        print("🚀 Starting Detailed MarkupLM Test")
        print("=" * 80)
...
def run_detailed_markuplm_test():
    """Run the detailed MarkupLM test."""
    print("🔍 Starting Detailed MarkupLM Test")
    print("=" * 80)
    
    try:
        test = DetailedMarkupLMTest()
        results = test.run_detailed_test()
        return results
    except Exception as e:
