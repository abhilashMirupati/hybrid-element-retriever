#!/usr/bin/env python3
"""
Enhanced Context Test - Better HTML Context for MarkupLM Differentiation

This test adds more contextual information to help MarkupLM distinguish between elements.
"""
...
class EnhancedContextTest:
    """Test that adds enhanced context to help MarkupLM distinguish between elements."""
    
    def __init__(self):
        """Initialize the test with real MarkupLM model."""
        print("🚀 Initializing Enhanced Context Test")
        print("=" * 80)
    ...
    def build_enhanced_html_context(self, node: Dict[str, Any], all_elements: List[Dict[str, Any]]) -> str:
        """Build enhanced HTML context with more information for MarkupLM differentiation."""
        print(f"🏗️  Building ENHANCED HTML context for node: {node.get('tag', 'unknown')} (ID: {node.get('attributes', {}).get('id', 'no-id')})")
        print(f"   Node text: '{node.get('text', '')}'")
        
        # Determine if this is main content or footer based on xpath
        node_xpath = node.get('xpath', '')
        is_footer = 'footer' in node_xpath.lower()
        is_main = 'main' in node_xpath.lower()
        
        print(f"   Context type: {'Footer' if is_footer else 'Main' if is_main else 'Unknown'}")
        
        # Build HTML structure with enhanced context
        html_parts = []
        
        # Add section context
        if is_footer:
            html_parts.append('<footer class="page-footer">')
            html_parts.append('<div class="footer-navigation">')
        elif is_main:
            html_parts.append('<main class="page-main">')
        ...
        if is_footer:
            html_parts.append('<nav class="footer-nav">')
            html_parts.append('<ul class="footer-links">')
            html_parts.append('<li class="footer-link-item">')
        elif is_main:
            html_parts.append('<nav class="main-nav">')
            html_parts.append('<ul class="main-links">')
            html_parts.append('<li class="main-link-item">')
        else:
            html_parts.append('<nav class="page-nav">')
            html_parts.append('<ul class="page-links">')
            html_parts.append('<li class="page-link-item">')
        ...
        print(f"   Built ENHANCED HTML context ({len(html_context)} chars)")
    ...
    def execute_step_with_markuplm(self, step: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Build ENHANCED HTML context for each node
            html_snippets = []
            for node in match_nodes:
                html_context = self.build_enhanced_html_context(node, all_elements)
    ...
    def run_enhanced_context_test(self):
        """Run the enhanced context test with multiple exact matches."""
        print("🚀 Starting Enhanced Context Test")
        print("=" * 80)
...
def run_enhanced_context_test():
    """Run the enhanced context test."""
    print("🔍 Starting Enhanced Context Test")
    print("=" * 80)
    
    try:
        test = EnhancedContextTest()
        results = test.run_enhanced_context_test()
        return results
    except Exception as e:
