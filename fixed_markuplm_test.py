#!/usr/bin/env python3
"""
Fixed MarkupLM Test - Correct Processor Usage
...
class FixedMarkupLMTest:
    """Fixed MarkupLM test with correct processor usage."""
    
    def __init__(self):
        """Initialize the test with real MarkupLM model."""
        print("🚀 Initializing Fixed MarkupLM Test")
        print("=" * 80)
        ...
        for idx, snippet in enumerate(html_snippets):
            ...
            try:
                ...
                with torch.no_grad():
                    ...
                    # Calculate scores
                    start_score = torch.softmax(start_logits, dim=-1).max().item()
                    end_score = torch.softmax(end_logits, dim=-1).max().item()
    ...
    def save_results(self, results: List[Dict[str, Any]]):
        ...
        filename = 'fixed_markuplm_results.json'
        with open(filename, 'w') as f:
            json.dump(results, f, indent=2)
        print(f"\n📄 Detailed results saved to: {filename}")


def run_fixed_markuplm_test():
    """Run the fixed MarkupLM test."""
    print("🔍 Starting Fixed MarkupLM Test")
    print("=" * 80)
    
    try:
        test = FixedMarkupLMTest()
