#!/usr/bin/env python3
"""
Simple MarkupLM Test - Direct Implementation

This test implements the exact workflow without HER dependencies:
...
class SimpleMarkupLMTest:
    """Simple MarkupLM test with direct implementation."""
    
    def __init__(self):
        """Initialize the test with real MarkupLM model."""
        print("🚀 Initializing Simple MarkupLM Test")
        print("=" * 80)
    ...
    def score_snippets_with_markuplm(self, html_snippets: List[str], query: str) -> List[Tuple[float, str, int]]:
        ...
        for idx, snippet in enumerate(html_snippets):
            ...
            try:
                ...
                print(f"      - Questions: {inputs['questions']}")
                print(f"      - HTML strings: {len(inputs['html_strings'])}")
                ...
                with torch.no_grad():
                    ...
                    print(f"      - Start logits max: {start_score:.3f}")
                    print(f"      - End logits max: {end_score:.3f}")
    ...
    def save_results(self, results: List[Dict[str, Any]]):
        ...
        filename = 'simple_markuplm_results.json'
        with open(filename, 'w') as f:
            json.dump(results, f, indent=2)
        print(f"\n📄 Detailed results saved to: {filename}")


def run_simple_markuplm_test():
    """Run the simple MarkupLM test."""
    print("🔍 Starting Simple MarkupLM Test")
    print("=" * 80)
    
    try:
        test = SimpleMarkupLMTest()
