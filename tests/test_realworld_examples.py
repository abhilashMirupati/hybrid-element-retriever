...
class TestRealWorldExamples:
    ...
    def test_spa_navigation_intent(self, examples_dir):
        ...
        has_url_change_check = any(
            "url_changed" in step.get("validation", {}) for step in intent_data["steps"]
        )
    ...
    def test_self_healer_with_dom_resnapshot(self, mock_page):
        ...
        result = healer.heal(failed_locator, page=mock_page, resnapshot_on_fail=True)
    ...
    def test_action_result_json_format(self, action, expected_fields):
        ...
        result = ActionResult(
            success=True, action=action, locator="//button[@id='test']", duration_ms=100
        )
    ...
    def test_error_recovery_flow(self, client):
        ...
        with patch.object(client.executor, "execute") as mock_execute:
            ...
            with patch.object(client.healer, "heal") as mock_heal:
                ...
                mock_heal.return_value.selected_locator = (
...
class TestEdgeCases:
    ...
    @pytest.mark.parametrize(
        "invalid_input",
        [None, "", "   ", "<script>alert('xss')</script>", "'; DROP TABLE users; --"],
    )
...
class TestPerformance:
    ...
    def test_fusion_scorer_batch_performance(self):
        ...
        elements = [
            {"tag": "button", "id": f"btn_{i}", "text": f"Button {i}"}
            for i in range(100)
        ]
