...
class TestRealWorldExamples:
    ...
    def test_login_with_modal(self):
        ...
        for intent in intents:
            result = self.client.query(
                intent["step"], url=f"file://{html_path.absolute()}"
            )
            ...
            ), f"Low confidence for {intent['id']}: {result['confidence']}"

            # Verify selector is unique
            is_unique = self.verifier.verify_uniqueness(
                self.client.executor.page, result["selector"]
            )
            assert (
                is_unique
            ), f"Non-unique selector for {intent['id']}: {result['selector']}"
    ...
    def test_spa_route_changes(self):
        ...
        for intent in intents[:3]:  # Test first few intents that trigger route changes
            ...
            assert result[
                "success"
            ], f"Action failed for {intent['id']}: {result.get('error')}"
    ...
    def test_shadow_dom_nested(self):
        ...
        for intent in intents:
            ...
            assert result[
                "selector"
            ], f"Could not find shadow DOM element for {intent['id']}"
            assert (
                result["confidence"] > 0.4
            ), f"Low confidence for shadow DOM element {intent['id']}"
    ...
    def test_iframe_forms(self):
        ...
        for intent in intents:
            ...
            assert result[
                "selector"
            ], f"Could not find element in iframe for {intent['id']}"
    ...
    def test_overlay_auto_dismiss(self):
        ...
        result = self.client.act("Click Sign In button to show modal", url=url)
        ...
        result = self.client.act("Click Forgot Password link", url=url)
    ...
    def test_occlusion_detection(self):
        ...
        try:
            ...
            if not result["success"]:
                assert (
                    "occluded" in result.get("error", "").lower()
                    or "overlay" in result.get("error", "").lower()
    ...
    def test_post_action_verification(self):
        ...
        if "verification" in result:
            ...
            assert verification.get("url_changed") or verification.get(
                "dom_changed"
        ...
        if "verification" in result:
            assert result["verification"].get(
                "dom_changed"
...
class TestEdgeCases:
    ...
    def test_delayed_element_loading(self):
        ...
        try:
            ...
            result = self.client.act("Click Loaded Button", url=url)
