...
class TestSelfHealer:
    ...
    def test_healing_strategies(self):
        ...
        assert healer._relax_exact_match("//button[text()='Submit']") == "//button[contains(text(),'Submit']"
...
class TestPromotionStore:
    ...
    def test_init_sqlite(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            store = PromotionStore(
                store_path=Path(tmpdir) / "promotions.db",
                use_sqlite=True
            )
    ...
    def test_init_json(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            store = PromotionStore(
                store_path=Path(tmpdir) / "promotions.json",
                use_sqlite=False
            )
