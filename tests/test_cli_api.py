...
class TestHybridClient:
    ...
    def test_act_basic(self, mock_executor):
        ...
        # Mock components
        client.parser.parse = Mock(return_value=Mock(
            action="click",
            target_phrase="submit button",
            args=None,
            confidence=0.9
        ))
    ...
    def test_query(self, mock_executor):
        ...
        client._find_candidates = Mock(return_value=[
            ({"backendNodeId": 1, "tag": "button", "text": "Submit"}, 0.9, {"explanation": "Best match"}),
            ({"backendNodeId": 2, "tag": "button", "text": "Cancel"}, 0.3, {"explanation": "Partial match"})
        ])
        
        client.synthesizer.synthesize = Mock(side_effect=[
            ["#submit-btn"],
            ["#cancel-btn"]
        ])
        
        client.verifier.find_unique_locator = Mock(side_effect=[
            "#submit-btn",
            "#cancel-btn"
        ])
    ...
    def test_find_candidates(self, mock_executor):
        ...
        mock_session = Mock()
        mock_session.vector_store.search = Mock(return_value=[
        ...
        with patch('her.cli_api.rank_by_heuristics') as mock_heuristics:
            ...
            # Mock fusion
            client.rank_fusion.fuse = Mock(return_value=[
                ({"backendNodeId": 1}, 0.7, {"explanation": "Fused"})
            ])
    ...
    def test_execute_with_recovery_success(self, mock_executor):
        ...
        mock_executor.return_value.click = Mock(return_value=Mock(
            success=True,
            locator="#submit",
            overlay_events=[],
            retries=0,
        ...
        result = client._execute_with_recovery(
            "click",
            "#submit",
            None,
            []
    ...
    def test_execute_with_recovery_self_heal(self, mock_executor):
        ...
        # First attempt fails
        mock_executor.return_value.click = Mock(side_effect=[
