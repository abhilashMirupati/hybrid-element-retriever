#!/usr/bin/env python3
"""
Verizon Flow Test with Detailed MiniLM and MarkupLM Scores
Shows top K lists and scores before heuristics for each step
"""
...
def run_verizon_detailed_test():
    """Run Verizon flow test with detailed scoring information"""
    print("🚀 VERIZON FLOW TEST WITH DETAILED SCORES")
    print("=" * 80)
...
def print_detailed_scores(step_name, query, result):
    print("\\n" + "="*80)
    print(f"📊 DETAILED SCORES FOR: {step_name}")
    print("="*80)
    print(f"Query: '{query}'")
    ...
    # Show candidates with detailed scores
    ...
    if candidates:
        ...
        for i, candidate in enumerate(candidates[:10]):  # Show top 10
            ...
            print(f"{i+1:2d}. Score: {score:6.3f} | Tag: {tag:8s} | Int: {interactive} | '{text}...'")
            print(f"     XPath: {xpath}...")
            print()
    else:
        print("\\n❌ No candidates found")

def run_detailed_verizon_flow():
    try:
        ...
        # Step 2: Click on Phones (corrected)
        ...
        print_detailed_scores("STEP 2: Phones Navigation", phones_query, phones_result)
        ...
        print_detailed_scores("STEP 3: Apple Filter", apple_filter_query, apple_filter_result)
        ...
        print_detailed_scores("STEP 4: iPhone 16 Pro Selection", iphone_query, iphone_result)
        ...
        print_detailed_scores("STEP 5: PDP Validation", pdp_validation_query, pdp_result)
        ...
        print_detailed_scores("STEP 6: 512GB Storage", storage_query, storage_result)
        
        print("\\n🎉 DETAILED VERIZON FLOW TEST COMPLETED!")
        print("=" * 80)
        print("✅ All 6 steps analyzed with detailed scoring")
...
if __name__ == "__main__":
    success = run_detailed_verizon_flow()
    exit(0 if success else 1)
"""
    
    # Run the test
    print(f"🚀 Starting detailed test at: {datetime.now().isoformat()}")
...
def main():
    """Main test function"""
    print("🚀 VERIZON DETAILED SCORING TEST")
    print("=" * 80)
    
    success = run_verizon_detailed_test()
