    ...
    try:
        if query is None or descriptors is None:
            ...
            try:
                ...
                if result and hasattr(result, 'xpath'):
                    print(f"✓ {description}: Handled (returned result)")
                else:
                    print(f"✓ {description}: Handled (returned empty)")
            except (TypeError, AttributeError) as e:
                failures.append(f"{description}: {type(e).__name__} on None input")
...
try:
    ...
    print("⚠ Some components not available for integration testing")
...
try:
    api_tests = [
        ("Pipeline returns consistent structure", 
         lambda: hasattr(pipeline.process("test", [{"tag": "div"}]), 'xpath')),
    ...
    print("⚠ Components not available for API testing")
...
for query, elements, scenario in problematic:
    try:
        ...
        if result and hasattr(result, 'xpath') and result.xpath:
