#!/usr/bin/env python3
"""
Fixed Exact Match Test - Proper Target Text Matching

This test fixes the exact matching logic to only get elements that exactly match the target text.
"""
...
class FixedExactMatchTest:
    """Fixed exact match test with proper target text matching."""
    
    def __init__(self):
        """Initialize the test with real MarkupLM model."""
        print("🚀 Initializing Fixed Exact Match Test")
        print("=" * 80)
    ...
    def find_exact_match_nodes(self, elements: List[Dict[str, Any]], target_text: str) -> List[Dict[str, Any]]:
        """Find exact match nodes based on target text - FIXED VERSION."""
        ...
        for element in elements:
            ...
            if target_text.lower() == element_text.lower():
                ...
                print(f"   ✅ Found EXACT text match: {element.get('tag', 'unknown')} - '{element_text}'")
                continue
            ...
            for attr_name in important_attrs:
                ...
                if isinstance(attr_value, str) and target_text.lower() == attr_value.lower():
                    matches.append(element)
                    print(f"   ✅ Found EXACT attribute match: {attr_name}='{attr_value}'")
                    break
    ...
    def run_fixed_test(self):
        """Run the fixed test with proper exact matching."""
        print("🚀 Starting Fixed Exact Match Test")
        print("=" * 80)
...
def run_fixed_exact_match_test():
    """Run the fixed exact match test."""
    print("🔍 Starting Fixed Exact Match Test")
    print("=" * 80)
    
    try:
        test = FixedExactMatchTest()
        results = test.run_fixed_test()
        return results
    except Exception as e:
