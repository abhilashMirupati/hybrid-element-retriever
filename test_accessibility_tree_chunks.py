#!/usr/bin/env python3
"""
Test passing entire accessibility tree to MarkupLM in 512-token chunks.
"""
...
def create_html_from_accessibility_tree(ax_tree, max_tokens=512):
    """
    Convert accessibility tree to HTML structure, splitting into chunks if needed.
    """
    def build_html_recursive(node, depth=0):
        """Recursively build HTML from accessibility tree node."""
        if not node:
            return ""
        ...
        # Build HTML
        if children:
            children_html = ''.join([build_html_recursive(child, depth + 1) for child in children])
    ...
    # Build full HTML
    full_html = build_html_recursive(ax_tree)
...
def test_accessibility_tree_chunks():
    """Test passing entire accessibility tree to MarkupLM in chunks."""
    print("🌳 Testing Accessibility Tree with MarkupLM Chunks")
    print("=" * 70)
    ...
    try:
        ...
        print(f"\n📸 Taking page snapshot with accessibility tree...")
        snapshot = runner._snapshot("https://www.verizon.com/")
        
        # Get accessibility tree
        ax_tree = snapshot.get("ax_tree")
        if not ax_tree:
            print("❌ No accessibility tree found in snapshot")
            return
        
        print(f"✅ Captured accessibility tree with {len(ax_tree.get('children', []))} root children")
        
        # Convert to HTML chunks
        print(f"\n🔧 Converting accessibility tree to HTML chunks...")
        html_chunks = create_html_from_accessibility_tree(ax_tree, max_tokens=512)
...
if __name__ == "__main__":
    test_accessibility_tree_chunks()