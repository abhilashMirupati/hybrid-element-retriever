...
class ONNXModelResolver:
    ...
    def __init__(
        self, 
        model_name: str,
        embedding_dim: int = FALLBACK_EMBEDDING_DIM
    ) -> None:
    ...
    def _load_model(self) -> None:
        ...
        if not self.model_path or not self.model_path.exists():
            logger.info(f"ONNX model '{self.model_name}' not found, using deterministic fallback")
            return
        ...
        try:
            ...
            if self.tokenizer_path and self.tokenizer_path.exists():
                try:
                    ...
                    logger.warning(f"Failed to load tokenizer: {e}, will use simple tokenization")
                    self.tokenizer = None
...
def get_element_resolver() -> ONNXModelResolver:
    ...
    return ONNXModelResolver(
        model_name=ELEMENT_MODEL_NAME,
        embedding_dim=ELEMENT_EMBEDDING_DIM
    )
