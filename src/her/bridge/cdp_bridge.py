...
def get_flattened_document(
    ...
    try:
        ...
        try:
            ...
            logger.debug(f"getFlattenedDocument not available, falling back to getDocument: {e}")
            
            # Get document root
            doc_response = client.send("DOM.getDocument", {
                "depth": depth,
                "pierce": pierce
            })
        ...
        for node in nodes:
            ...
            if node_id and node.get("nodeType") == 1:  # Element nodes only
                try:
                    ...
                    logger.debug(f"Could not get styles for node {node_id}: {style_error}")
                    styles[node_id] = {}
