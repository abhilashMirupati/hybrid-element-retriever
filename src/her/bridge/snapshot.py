...
def merge_dom_and_ax(
    dom_nodes: List[Dict[str, Any]], ax_nodes: List[Dict[str, Any]]
) -> List[Dict[str, Any]]:
...
def capture_frame_snapshot(
    ...
    try:
        ...
        for f in page.frames:
            if f.url == frame_url or f.name == frame_info.get("frame", {}).get(
                "name", ""
            ):
                pw_frame = f
                break
    ...
    for i, child in enumerate(child_frames):
        ...
        child_descriptors = capture_frame_snapshot(
            page, child, child_path, visited_frames
        )
...
def capture_snapshot(
    page: Optional[Page], include_frames: bool = True, frame_path: str = ""
    ...
    try:
        ...
        if include_frames:
            ...
            if frame_tree:
                ...
                for i, child_frame in enumerate(child_frames):
                    child_path = f"{frame_path}.{i}" if frame_path else str(i)
                    frame_descriptors = capture_frame_snapshot(
                        page, child_frame, child_path
                    )
                    descriptors.extend(frame_descriptors)
        ...
        logger.info(
            f"Captured snapshot: {len(descriptors)} elements, hash={dom_hash[:8]}"
        )
