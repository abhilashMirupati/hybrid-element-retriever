...
class SQLiteCache:
    ...
    def __init__(self, db_path: Optional[Path] = None, max_size_mb: int = DISK_CACHE_SIZE_MB):
    ...
    def get(self, key: str) -> Optional[Any]:
        ...
        with self.lock:
            with sqlite3.connect(str(self.db_path)) as conn:
                cursor = conn.execute(
                    "SELECT value, hits FROM cache WHERE key = ?",
                    (key,)
                )
                row = cursor.fetchone()
    ...
    def _evict_lru(self, conn: sqlite3.Connection, needed_bytes: int) -> None:
        ...
        if keys_to_evict:
            ...
            conn.execute(f"DELETE FROM cache WHERE key IN ({placeholders})", keys_to_evict)
