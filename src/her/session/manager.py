...
class SessionManager:
    ...
    def create_session(
        ...
        session = SessionState(
            ...
            auto_index_enabled=auto_index if auto_index is not None else self.auto_index
        )
    ...
    def destroy_session(self, session_id: str) -> None:
        ...
        if session_id in self.sessions:
            ...
            pages_to_remove = [
                p for p, sid in self._page_to_session.items()
                if sid == session_id
            ]
    ...
    def should_reindex(
        ...
        if self.reindex_on_change and page:
            ...
            if detect_dom_change(session.dom_hash, new_hash):
                logger.debug(f"DOM changed: {session.dom_hash[:8]}... -> {new_hash[:8]}...")
                return True
    ...
    def export_session(self, session_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_indexed": session.last_indexed.isoformat() if session.last_indexed else None,
            "url": session.url,
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        """
        total_elements = sum(
            len(s.element_descriptors) for s in self.sessions.values()
        )
