...
from .rank.heuristics import rank_by_heuristics, explain_heuristic_score
    ...
    def _execute_with_recovery(
        ...
        if not result.success and self.executor.page:
            healed_locators = self.healer.heal(
                locator,
                self.executor.page,
                context={"action": action, "args": args}
            )
            ...
            for candidate, _, _ in candidates[1:4]:  # Try next 3 candidates
                ...
                for alt_locator in alt_locators:
                    ...
                    if alt_result.success:
                        alt_result.verification["recovery_method"] = "alternative_candidate"
                        return alt_result
