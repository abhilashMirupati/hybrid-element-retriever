...
class IntentParser:
    ...
    def _init_action_patterns(self) -> List[Tuple[re.Pattern, str, str]]:
        ...
        return [
            ...
            # Type/Fill actions
            (re.compile(r'^type\s+["\'](.+?)["\']\s+(?:in|into)\s+(.+)$', re.I), 'type', 'text'),
            (re.compile(r'^enter\s+["\'](.+?)["\']\s+(?:in|into)\s+(.+)$', re.I), 'type', 'text'),
            (re.compile(r'^fill\s+(.+?)\s+with\s+["\'](.+?)["\']$', re.I), 'type', 'text_reversed'),
            (re.compile(r'^input\s+["\'](.+?)["\']\s+(?:in|into)\s+(.+)$', re.I), 'type', 'text'),
            (re.compile(r'^type\s+(.+)$', re.I), 'type', 'infer'),
            
            # Select actions
            (re.compile(r'^select\s+["\'](.+?)["\']\s+from\s+(.+)$', re.I), 'select', 'option'),
            (re.compile(r'^choose\s+["\'](.+?)["\']\s+(?:from|in)\s+(.+)$', re.I), 'select', 'option'),
            (re.compile(r'^pick\s+["\'](.+?)["\']\s+(?:from|in)\s+(.+)$', re.I), 'select', 'option'),
            ...
            (re.compile(r'^wait\s+until\s+(.+?)\s+(?:is\s+)?visible$', re.I), 'wait', 'visible'),
            (re.compile(r'^wait\s+(\d+)\s*(?:seconds?|secs?|s)$', re.I), 'wait', 'time'),
            (re.compile(r'^pause\s+(?:for\s+)?(\d+)\s*(?:seconds?|secs?|s)?$', re.I), 'wait', 'time'),
            
            # Navigation actions
            (re.compile(r'^(?:go\s+to|navigate\s+to|open)\s+(.+)$', re.I), 'navigate', None),
            (re.compile(r'^refresh(?:\s+the\s+page)?$', re.I), 'refresh', None),
            (re.compile(r'^go\s+back$', re.I), 'back', None),
            
            # Assertion actions
            (re.compile(r'^verify\s+(.+?)\s+(?:is\s+)?visible$', re.I), 'assert', 'visible'),
            (re.compile(r'^check\s+(?:that\s+)?(.+?)\s+exists$', re.I), 'assert', 'exists'),
    ...
    def _infer_intent(self, step: str) -> Intent:
        ...
        action_words = ['click', 'press', 'tap', 'type', 'enter', 'select', 'hover', 'wait']
        target = step
    ...
    def explain_intent(self, intent: Intent) -> str:
        ...
        explanations = {
            ...
            'type': f"Type '{intent.args}' into '{intent.target_phrase}'" if intent.args else f"Type into '{intent.target_phrase}'",
            'select': f"Select '{intent.args}' from '{intent.target_phrase}'" if intent.args else f"Select '{intent.target_phrase}'",
            'hover': f"Hover over '{intent.target_phrase}'",
            'wait': self._explain_wait(intent),
            'assert': f"Verify that '{intent.target_phrase}' {intent.constraints.get('type', 'exists')}" if intent.constraints else f"Assert '{intent.target_phrase}'",
        ...
        explanation = explanations.get(intent.action, f"{intent.action} '{intent.target_phrase}'")
