...
class PromotionStore:
    ...
    def _load_from_sqlite(self) -> None:
        ...
        for row in cursor.fetchall():
            ...
            self.cache[key] = PromotionRecord(
                ...
                failure_count=row[3],
                score=row[4],
                last_used=datetime.fromisoformat(row[5]) if isinstance(row[5], str) else datetime.now(),
                metadata=metadata
            )
        ...
        try:
            ...
            for key, record_data in data.items():
                self.cache[key] = PromotionRecord(
                    ...
                    last_used=datetime.fromisoformat(record_data.get('last_used', datetime.now().isoformat())),
    ...
    def clear(self, context: Optional[str] = None) -> int:
        ...
        if self.use_sqlite:
            ...
            cursor = conn.cursor()
            if context:
                cursor.execute("DELETE FROM promotions WHERE context LIKE ?", (f"%{context}%",))
            else:
