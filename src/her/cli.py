...
"""
    ...
    # Act command
    act_parser = subparsers.add_parser("act", help="Execute an action on a page")
    act_parser.add_argument("step", help="Natural language step to execute")
    act_parser.add_argument(
        "--url", help="URL to navigate to before executing", default=None
    )
    act_parser.add_argument(
        "--headless",
        action="store_true",
    ...
    query_parser = subparsers.add_parser(
        "query", help="Query for elements without executing"
    )
    query_parser.add_argument("phrase", help="Search phrase for elements")
    query_parser.add_argument(
        "--url", help="URL to navigate to before querying", default=None
    )
    query_parser.add_argument(
        "--headless",
        action="store_true",
    ...
    query_parser.add_argument(
        "--limit", type=int, help="Maximum number of results", default=10
    )
    query_parser.add_argument(
        "--output",
        help="Output format (json, text)",
    ...
    cache_parser.add_argument(
        "--clear", action="store_true", help="Clear all cached embeddings"
    )
    cache_parser.add_argument(
...
def handle_cache_command(args):
    ...
    if args.clear:
        ...
        stats = {
            "embedding_cache": {"path": str(cache_dir), "exists": cache_dir.exists()},
            "promotion_store": {
...
def output_result(result, format="json"):
    ...
    if format == "json":
        ...
        if isinstance(result, dict):
            ...
            clean_result = {
                k: v for k, v in result.items() if v is not None and v != ""
            }
